...
function method_child_cmb2_tinymce_options( $temp, $class = '' ) {
	$output = array();
	if ( ! empty( $class ) ) {
...
*/
function method_child_cmb2_metabox_include_default_page( $display, $meta_box ) {
...
add_filter( 'cmb2_show_on', 'method_child_cmb2_metabox_include_default_page', 10, 2 );
 ...
 * @return bool display metabox
 */
function method_child_cmb2_metabox_include_front_page( $display, $meta_box ) {
	if ( ! isset( $meta_box['show_on']['key'] ) ) {
		return $display;
	}
...
add_filter( 'cmb2_show_on', 'method_child_cmb2_metabox_include_front_page', 10, 2 );
